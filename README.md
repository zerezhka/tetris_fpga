...
| 8 | MiSTer hardware bring-up (OSD ROM loading, LCD renderer, audio out) | 🚧 smoke-test instantiation only |
...
`HT943.sv` currently instantiates the core with pins tied to pull-ups — enough
to prove it synthesizes, fits, and routes. Still to do: ROM upload via OSD
(`.brick` mask options → core parameters become runtime registers), an LCD
segment renderer reading the RAM like `_renderVRAM` does, audio PCM output,
and input mapping.
