...
Статус: **ОТГРУЖЕНО и проверено на железе** (2026-07-06). Все шаги 1–5
сделаны: формат v3, тулинг, RTL-лоадер, HT943-микс, PAK первым в CONF_STR
(1-click UX), регрессия all-PASS, Quartus-пересборка (0 errors, clk_sys
Fmax 55.74 МГц), деплой `HT943_20260706_1241.rbf` + 6 v3-паков на
`root@192.168.1.235`. Проверка: pak-only mgl (только SpaceIntruder.pak, без
.bin/.sro) → игра запустилась из встроенного ROM (скриншот с активной
формацией инвейдеров). Цель достигнута: `.pak` — самодостаточный картридж,
один файл поднимает программу+звук+лицо+профиль, без mgl и без 3 файлов.
Заодно ожил весь ранее-dormant стек (fine-mask, configurable ghost, E23-рамка).
